...
순위표를 선택하면 해당 Herdr 터미널로 바로 이동할 수 있습니다.
...
상태 변화가 화면 하단에 피트월 무전으로 표시됩니다. 피트인, 사고, 체커기 같은
순간들입니다. 무전을 선택하면 해당 agent의 Herdr 터미널로 이동합니다.

무전 문구는 어떤 상태 변화가 일어났는지에 따라 정해진 대본에서 고른 연출이며,
랩이나 포인트와 마찬가지로 허구입니다. **agent가 실제로 한 말이 아닙니다** —
대시보드는 agent의 발화를 읽을 수 없습니다.
